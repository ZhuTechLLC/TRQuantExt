...
app = typer.Typer(help="QMT Bridge CLI - 快速查询策略/回测/交易信息")
console = Console()

QMT_BRIDGE_URL = "http://127.0.0.1:8100"


def _get(endpoint: str, params: Optional[dict] = None):
    resp = requests.get(f"{QMT_BRIDGE_URL}{endpoint}", params=params)
...
@app.command()
def refresh():
    resp = requests.post(f"{QMT_BRIDGE_URL}/refresh")
    resp.raise_for_status()
