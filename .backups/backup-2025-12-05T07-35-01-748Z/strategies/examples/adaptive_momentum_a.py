# -*- coding: utf-8 -*-
"""
A股自适应动量策略 - 基于市场环境自动识别和策略适配
适用于A股市场，使用沪深300、创业板指等作为基准
"""
import pandas as pd
import numpy as np
...
class AdaptiveMomentumStrategyA(BaseStrategy):
    """
    A股自适应动量策略
    ...
    def __init__(
        ...
        growth_index: str = "399006.XSHE",  # 创业板指
        ...
        Args:
            ...
            take_profit: 止盈比例
        """
        super().__init__(name="Adaptive Momentum Strategy A-Share")
    ...
    def handle_data(self, data: pd.DataFrame, date: datetime):
        ...
        if self._should_rebalance(date):
            ...
            for candidate in candidates[:buy_count]:
                ...
                amount = int(target_value / current_price / 100) * 100  # 按手买入（A股100股为1手）
